...
public class AdminController {
    ...
    public Map<String, Object> addCourse(@RequestBody Course course) {
        ...
        if (rows > 0) {
            try { ragSyncService.syncCourse(course.getId()); } catch (Exception e) { log.warn("RAG自动同步失败: courseId={}", course.getId(), e); }
            return ok("添加成功", course);
        }
        return fail("添加失败");
    }
    ...
    public Map<String, Object> deleteCourse(@PathVariable Long id) {
        ...
        if (rows > 0) {
            try { ragSyncService.syncCourse(id); } catch (Exception e) { log.warn("RAG自动同步失败: courseId={}", id, e); }
            return ok("删除成功", null);
        }
        return fail("删除失败");
    }
    ...
    @PostMapping("/course/package/add")
    public Map<String, Object> addPackage(@RequestBody CoursePackage pkg) {
        int rows = coursePackageService.add(pkg);
        return rows > 0 ? ok("添加成功", pkg) : fail("添加失败");
    }

    @PutMapping("/course/package/update")
    public Map<String, Object> updatePackage(@RequestBody CoursePackage pkg) {
        int rows = coursePackageService.update(pkg);
        return rows > 0 ? ok("修改成功", pkg) : fail("修改失败");
    }

    @DeleteMapping("/course/package/delete/{id}")
    public Map<String, Object> deletePackage(@PathVariable Long id) {
        int rows = coursePackageService.deleteById(id);
        return rows > 0 ? ok("删除成功", null) : fail("删除失败");
    }

    @PostMapping("/course/package/item/add")
    public Map<String, Object> addPackageItem(@RequestBody PackageItem item) {
        int rows = packageItemService.add(item);
        return rows > 0 ? ok("添加成功", item) : fail("添加失败");
    }

    @DeleteMapping("/course/package/item/delete/{id}")
    public Map<String, Object> deletePackageItem(@PathVariable Long id) {
        int rows = packageItemService.deleteById(id);
        return rows > 0 ? ok("删除成功", null) : fail("删除失败");
    }
